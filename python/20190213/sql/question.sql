...
select * from (
  select t.s_id, group_concat(t.c_id) c_ids from (select s_id, c_id from Score) t where s_id != '01' group by t.s_id
) co
where co.c_ids = (select group_concat(t.c_id) c_ids from (select s_id, c_id from Score) t where s_id = '01' group by t.s_id);
